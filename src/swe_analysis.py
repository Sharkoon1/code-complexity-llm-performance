...
CLASSICAL = [
    ...
    "loc", "lloc", "nesting_max",
    "token_count", "n_tokens_meaningful", "function_count", "n_functions", "n_branches",
]
LM_CC_FEATURES = [
...
def correlation_report(clean, score="resolved", control="loc"):
    ...
    return full_correlation_table(clean, _available(clean), score=score, control=control)


def extremes(clean, n=5):
    cols = [c for c in ["instance_id", "lm_cc_score", "loc", "cc_sum", "nesting_max"] if c in clean.columns]
    for label, mask in [("resolved", clean["resolved"] == 1), ("not resolved", clean["resolved"] == 0)]:
        for which, sub in [("High", clean[mask].nlargest(n, "lm_cc_score")),
...
def metrics_overview(clean, title=""):
    ...
    plt.xlabel("LM-CC"); plt.ylabel("count"); plt.title(f"LM-CC distribution {title}".strip())
    plt.show()
    ...
    for ax, m in zip(axes, ["lm_cc_score", "nesting_max"]):
        ...
        ax.set_xlabel("LOC"); ax.set_ylabel(m); ax.set_title(f"{m} vs LOC (rho={rho:.3f})")
    plt.tight_layout()
    plt.show()


def per_agent_table(long, top=10):
    table = per_agent_correlation_table(long, metric="lm_cc_score", score="resolved", control="loc")

    def _fmt(df):
        df = df.copy()
        df["agent"] = df["agent"].map(lambda n: n.split("_", 1)[1] if n[:8].isdigit() else n)
        df["resolve_rate"] = (df["resolve_rate"] * 100).round(1).astype(str) + " %"
        for c in ["sample_zero", "sample_partial_loc", "subgroup_zero", "subgroup_partial_loc"]:
...
def all_agents_report(result_path, title=""):
    ...
    long = all_predictions.merge(clean[["instance_id", *LONG_METRIC_COLS]], on="instance_id", how="inner")
    ...
    plt.xlabel("resolution rate"); plt.ylabel("count")
    ...
    clean["tier"] = pd.cut(clean["resolution_rate"], [-0.01, 0.33, 0.66, 1.01],
                           labels=["hard", "medium", "easy"])
    print(clean.groupby("tier", observed=True)["lm_cc_score"].agg(["count", "mean", "median"]).to_string())
    ...
    plt.suptitle(""); plt.title(f"LM-CC by difficulty tier {title}".strip()); plt.ylabel("LM-CC")
    plt.show()
...
def whole_vs_function_report():
    ...
    print("rho(LM-CC, LOC):")
    print(f"  whole file : {spearmanr(clean['lm_cc_score_file'], clean['loc_file']).correlation:+.3f}")
    print(f"  function   : {spearmanr(clean['lm_cc_score_fn'], clean['loc_fn']).correlation:+.3f}")
    ...
    print(full_correlation_table(clean, ["lm_cc_score_file", "cc_sum_file", "nesting_max_file"],
                                 score="resolution_rate", control="loc_file").to_string(index=False))
    print("\n=== FUNCTION (control = loc_fn) ===")
    print(full_correlation_table(clean, ["lm_cc_score_fn", "cc_sum_fn", "nesting_max_fn"],
                                 score="resolution_rate", control="loc_fn").to_string(index=False))
